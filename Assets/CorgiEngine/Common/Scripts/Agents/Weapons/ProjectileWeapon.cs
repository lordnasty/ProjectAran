...
namespace MoreMountains.CorgiEngine
{
    ...
    /// </summary>
    [MMHiddenProperties("WeaponOnMissFeedback", "ApplyRecoilOnHitDamageable", "ApplyRecoilOnHitNonDamageable",
    ...
    public class ProjectileWeapon : Weapon
    {
        ...
        [MMReadOnly] [Tooltip("the local position at which this projectile weapon should spawn projectiles")]
        ...
        private bool bUsesMultiPooler = false;
        ...
        protected override void WeaponUse()
        {
            ...
            if (bUsesMultiPooler)
            {
                if (ObjectPooler.GetComponent<MMMultipleObjectPooler>().Pool[0] == ObjectPooler.GetComponent<MMMultipleObjectPooler>().GetCurrentPoolerObject())
                {
                    if (ObjectPooler.GetComponent<MMMultipleObjectPooler>().GetCurrentIndex() > 20)
                    {
                        ObjectPooler.GetComponent<MMMultipleObjectPooler>().ResetCurrentIndex();
                    }
               
                }

                if (ObjectPooler.GetComponent<MMMultipleObjectPooler>().Pool[1].Enabled)
                {
                    if (ObjectPooler.GetComponent<MMMultipleObjectPooler>().Pool[1] == ObjectPooler.GetComponent<MMMultipleObjectPooler>().GetCurrentPoolerObject())
                    {
                    
                        if (ObjectPooler.GetComponent<MMMultipleObjectPooler>().GetCurrentIndex() > 40)
                        {
                            ObjectPooler.GetComponent<MMMultipleObjectPooler>().UpdateCurrentIndex(21);
                        }
            ...
            bool triggerObjectActivation = true)
        {
            ...
            if (projectile != null)
            {
                ...
                projectile.SetDirection(spread * transform.right * (Flipped ? -1 : 1), transform.rotation,
                    Owner.IsFacingRight);
        ...
        {
            _spawnPositionCenter = (ProjectileSpawnTransform == null)
                ? this.transform.position
                : ProjectileSpawnTransform.transform.position;
